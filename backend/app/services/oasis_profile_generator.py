...
class OasisProfileGenerator:
    ...
    INDIVIDUAL_ENTITY_TYPES = [
        ...
        "celebactor", "actor", "star", "celebrity", "individual" # 确保这些标签都被识别为个人
    ]
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        ...
        return base_prompt
    # --- 修改点 2: 完善个人人设 Prompt (确保包含 country) ---
    def _build_individual_persona_prompt(
        ...
        """构建个人实体的恋爱匹配版详细人设提示词 - 补全 country 字段"""
        attrs_str = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "无"
        context_str = context[:4000] if context else "无额外上下文"
        
        return f"""为实体生成详细的**恋爱匹配与社交网络用户人设**。
...
1. bio: 社交媒体简介，200字左右（展现独特的个人魅力、情感态度或当前的心情钩子）。
2. persona: 详细人设描述（2000字的纯文本），需包含:
   - **基本信息与特质**：年龄、职业、教育背景、所在地、**详细星盘（太阳、上升、月亮星座）**、外形风格描述。
   - **性格灵魂**：**MBTI类型**、核心性格、情绪表达方式、**依恋类型（如安全型、回避型等）**。
   - **恋爱履历**：过往恋爱经历（初恋、刻骨铭心的经历、遗憾、成长点）、目前的情感状态、对前任的态度。
   - **恋爱观与立场**：对热门恋爱话题的态度（如：异地恋、婚前同居、AA制、闪婚）、最容易被激怒或感动的瞬间。
   - **社交行为**：在恋爱/社交软件上的表现（回复速度、互动习惯、朋友圈/动态内容偏好、常用的语言风格）。
   - **独特特征**：口头禅、特殊经历、深度个人爱好（如：小众运动、收藏癖）。
   - **社交记忆**：人设的重要部分，介绍该个体在当前模拟社交圈中的已有动作、反应、心仪对象或已有的互动足迹。
3. age: 年龄数字（必须是整数，建议18-60）。
4. gender: 性别，必须是英文: "male" 或 "female"。
5. mbti: MBTI类型（如 INTJ、ENFP 等）。
6. zodiac: 星座或核心星盘（使用中文，如"天蝎座/上升狮子"）。
7. relationship_status: 情感状态（如"单身"、"暧昧中"、"渴望恋爱"等）。
8. dating_philosophy: 恋爱观核心标签（如"纯爱战神"、"自由主义"、"理性择偶"）。
9. profession: 职业。
10. country: 国家（必须填，例如"中国"） <-- 核心修复：框架强依赖此字段。
11. interested_topics: 感兴趣话题数组。

重要:
- 所有字段值必须是字符串或数字，不要使用换行符。
- persona 必须是一段连贯的文字描述，严禁使用列表或小标题。
- 使用中文（除了 gender 字段必须用英文 male/female，mbti 必须用英文代号）。
- 内容要与实体信息保持一致。
